...
export async function buildServer(env: ApiEnv, prisma: PrismaClient) {
  ...
  // ===== ONLYOFFICE editor integration (Phase B) =====
  //
  //   GET  /files/:id/onlyoffice/config       — config for the iframe (admin/user)
  //   GET  /files/:id/onlyoffice/raw          — ONLYOFFICE fetches the document
  //   POST /files/:id/onlyoffice/callback     — ONLYOFFICE pushes saved version back
  //
  // ONLYOFFICE Document Server doesn't see our session cookie. We auth its
  // document-fetch with a short-lived HMAC token in the URL. We trust its
  // callback by verifying the JWT it signs with ONLYOFFICE_JWT_SECRET.

  app.get<{ Params: { id: string } }>("/files/:id/onlyoffice/config", async (request, reply) => {
    const user = await requireUser(prisma, request, reply);
    if (!user) return;
    const file = await prisma.file.findUnique({
      where: { id: request.params.id },
    ...
    if (file.status === FileStatus.TRASHED) return reply.code(409).send({ error: "file in trash" });
    ...
    if (!latest) return reply.code(409).send({ error: "no version yet" });

    const onlyofficeJwtSecret = env.onlyofficeJwtSecret;
    if (!onlyofficeJwtSecret) {
      return reply.code(503).send({ error: "ONLYOFFICE not configured (set ONLYOFFICE_JWT_SECRET)" });
    }

    // Short-lived (10 min) signed token so ONLYOFFICE can pull /raw without cookies.
    const docToken = signOfficeDocToken({
      fileId: file.id,
      versionId: latest.id,
      exp: Math.floor(Date.now() / 1000) + 600
    }, env.cookieSecret);

    const internalApi = process.env.ONLYOFFICE_INTERNAL_API_URL ?? "http://api:4000";
    const fileType = "docx";
    const documentKey = `${file.id}-${latest.id}`;
    // ONLYOFFICE config. document.key MUST change whenever the file changes
    // (so the editor server reloads), hence we tie it to versionId.
    const docConfig = {
      document: {
        fileType,
        key: documentKey,
        title: file.name,
        url: `${internalApi}/files/${file.id}/onlyoffice/raw?docToken=${encodeURIComponent(docToken)}`,
        permissions: { edit: true, download: true, print: true }
      },
      documentType: "word",
      editorConfig: {
        callbackUrl: `${internalApi}/files/${file.id}/onlyoffice/callback?docToken=${encodeURIComponent(docToken)}`,
        mode: "edit",
        lang: "zh-CN",
    ...
    // ONLYOFFICE expects the whole config signed when JWT is enabled.
    const token = signOfficeConfigJwt(docConfig, onlyofficeJwtSecret);
    return { config: { ...docConfig, token } };
  });

  // Pulled by the ONLYOFFICE document-server container itself (no user cookies).
  app.get<{ Params: { id: string }; Querystring: { docToken?: string } }>(
    "/files/:id/onlyoffice/raw",
    async (request, reply) => {
      const payload = verifyOfficeDocToken(request.query.docToken ?? "", env.cookieSecret);
      if (!payload || payload.fileId !== request.params.id) {
        return reply.code(401).send({ error: "invalid or expired doc token" });
      }
      ...
      if (!version) return reply.code(404).send({ error: "no version" });

      const plaintext = await decryptLatestVersion(prisma, env, version);
      reply
        .header("content-type", file.mimeType || "application/octet-stream")
        .header("content-length", plaintext.byteLength.toString());
      return reply.send(plaintext);
    }
  );

  // ONLYOFFICE periodically POSTs save events here. status meanings:
  //   0: not used in our flow
  //   1: editing in progress (heartbeat) — no save
  //   2: ready to save (user closed all sessions)
  //   3: save error
  //   4: closed without changes
  //   6: force save (autosave / forcesave tick)
  //   7: force save error
  // For 2 and 6 we download from url and persist as a new FileVersion.
  app.post<{ Params: { id: string }; Querystring: { docToken?: string }; Body: unknown }>(
    "/files/:id/onlyoffice/callback",
    async (request, reply) => {
      const payload = verifyOfficeDocToken(request.query.docToken ?? "", env.cookieSecret);
      if (!payload || payload.fileId !== request.params.id) {
        return reply.code(401).send({ error: -1, message: "invalid doc token" });
      }

      // Verify the ONLYOFFICE-signed JWT sitting in the Authorization header.
      const onlyofficeSecret = env.onlyofficeJwtSecret;
      if (!onlyofficeSecret) {
        return reply.code(503).send({ error: -1, message: "onlyoffice not configured" });
      }
      const auth = request.headers["authorization"];
      let body: { status?: number; url?: string; key?: string } = (request.body as any) ?? {};
      if (typeof auth === "string" && auth.startsWith("Bearer ")) {
        const jwt = auth.slice(7);
        const verified = verifyOfficeConfigJwt<{ payload?: typeof body }>(jwt, onlyofficeSecret);
        if (!verified) {
          return reply.code(401).send({ error: -1, message: "invalid onlyoffice jwt" });
        }
        // ONLYOFFICE wraps the body inside the JWT payload too. Prefer the
        // signed copy, fall back to raw body.
        if (verified && typeof verified === "object" && "payload" in verified && verified.payload) {
          body = verified.payload as typeof body;
        }
      }

      const status = body.status ?? -1;
      // Save on real save events. Heartbeats / no-change closes are ignored.
      if ((status === 2 || status === 6) && body.url) {
        try {
          const res = await fetch(body.url);
          if (!res.ok) throw new Error(`fetch saved doc: HTTP ${res.status}`);
          ...
          if (!file) throw new Error("file disappeared");
          ...
          await replaceFileContent(prisma, env, file.id, buf, policy);
          console.info(`onlyoffice: saved new version for ${request.params.id} (${buf.byteLength} bytes)`);
        } catch (err) {
          console.error("onlyoffice callback save failed", err);
          return reply.send({ error: 1, message: errorMessage(err) });
        }
      }
...
// =========================================================================
// ONLYOFFICE JWT helpers (Phase B)
// =========================================================================
//
// Tiny HS256 implementations so we don't pull in jsonwebtoken just for this.
// Two flavors:
//   - signOfficeDocToken / verifyOfficeDocToken: our own short-lived token
//     in the URL that ONLYOFFICE uses to fetch the doc / call the callback.
//     Signed with COOKIE_SECRET because we already have it everywhere.
//   - signOfficeConfigJwt / verifyOfficeConfigJwt: the JWT format ONLYOFFICE
//     itself expects. Both sides share ONLYOFFICE_JWT_SECRET.
...
function signOfficeDocToken(payload: { fileId: string; versionId: string; exp: number }, secret: string): string {
  const head = b64url(JSON.stringify({ alg: "HS256", typ: "doctok" }));
  const body = b64url(JSON.stringify(payload));
  const sig = b64url(createHmac("sha256", secret).update(`${head}.${body}`).digest());
  return `${head}.${body}.${sig}`;
}

function verifyOfficeDocToken(token: string, secret: string): { fileId: string; versionId: string; exp: number } | null {
  ...
  try {
    const payload = JSON.parse(b64urlDecode(parts[1]).toString("utf8")) as { fileId: string; versionId: string; exp: number };
    if (typeof payload.exp !== "number" || payload.exp < Math.floor(Date.now() / 1000)) return null;
    if (typeof payload.fileId !== "string" || typeof payload.versionId !== "string") return null;
    return payload;
  } catch {
    return null;
  }
...
function signOfficeConfigJwt(payload: unknown, secret: string): string {
...
function verifyOfficeConfigJwt<T = unknown>(token: string, secret: string): T | null {
  if (!token) return null;
  const parts = token.split(".");
  if (parts.length !== 3) return null;
  const expectedSig = b64url(createHmac("sha256", secret).update(`${parts[0]}.${parts[1]}`).digest());
